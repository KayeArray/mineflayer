...
function lightweightBlockAt(absolutePoint) {
    // Inline chunk coordinate calculation
    const chunkX = Math.floor(absolutePoint.x / 16) * 16;
    const chunkZ = Math.floor(absolutePoint.z / 16) * 16;
    const column = bot.world.getColumnAt(absolutePoint);
    if (!column) return null;

    const x = Math.floor(absolutePoint.x) & 15;
    const y = Math.floor(absolutePoint.y);
    const z = Math.floor(absolutePoint.z) & 15;
    
    const stateId = column.getBlockStateId({ x, y, z });
    if (!stateId) return null;

    // Use cached block data if available
    let blockEnum = blockStateCache.get(stateId);
    if (!blockEnum) {
        blockEnum = bot.registry.blocksByStateId[stateId];
        if (!blockEnum) return null;
        blockStateCache.set(stateId, blockEnum);
    }

    return {
        name: blockEnum.name,
        position: absolutePoint.floored(),
        shapes: blockEnum.shapes
    };
}

function clearBlockStateCache() {
  ...
  bot.blockAt = blockAt
  bot.lightweightBlockAt = lightweightBlockAt 
